...
class Loader {
	...
	public function enqueue_customizer_controls() {
		...
		wp_localize_script(
			...
			apply_filters(
				...
				array(
					...
					'dashUpdatesMessage'            => sprintf( 'Please %s to the latest version of Neve Pro to manage the conditional headers.', '<a href="' . esc_url( admin_url( 'update-core.php' ) ) . '">' . __( 'update', 'nueve4' ) . '</a>' ),
		...
		wp_register_style( 'react-controls', $bundle_path . 'style-controls.css', [ 'nueve4-components' ], $dependencies['version'] );
		...
		if ( isset( $dependencies['chunks'] ) ) {
			foreach ( $dependencies['chunks'] as $chunk_file ) {
				...
				$chunk_handle = 'nueve4-customizer-chunk-' . $chunk_file;
