<?php
/**
 * Blog layout section.
 *
 * Author:          Andrei Baicus <andrei@themeisle.com>
 * Created on:      20/08/2018
 *
 * @package Nueve4\Customizer\Options
 */

namespace Nueve4\Customizer\Options;

use Nueve4\Customizer\Base_Customizer;
use Nueve4\Customizer\Defaults\Layout;
use Nueve4\Customizer\Types\Control;
use Nueve4\Customizer\Types\Section;

/**
 * Class Layout_Blog
 *
 * @package Nueve4\Customizer\Options
 */
class Layout_Blog extends Base_Customizer {
	use Layout;

	/**
	 * Holds the section name.
	 *
	 * @var string $section
	 */
	private $section = 'nueve4_blog_archive_layout';

	/**
	 * Function that should be extended to add customizer controls.
	 *
	 * @return void
	 */
	public function add_controls() {
		$this->section_blog();
		$this->add_layout_controls();
		$this->add_featured_post();
		$this->add_content_ordering_controls();
		$this->add_post_meta_controls();
		$this->add_typography_shortcut();

		add_action( 'customize_register', [ $this, 'adapt_old_pro' ], PHP_INT_MAX );
	}

	/**
	 * Adapting old pro versions to make them still usable with the old theme version.
	 */
	public function adapt_old_pro() {
		if ( ! defined( 'NEVE_PRO_VERSION' ) ) {
			return;
		}

		if ( version_compare( NEVE_PRO_VERSION, '1.2.8', '>' ) ) {
			return;
		}

		$changes = [
			'nueve4_posts_order'                   => [ 'priority' => 51 ],
			'nueve4_post_content_ordering'         => [ 'active_callback' => '__return_true' ],
			'nueve4_blog_ordering_content_heading' => [ 'controls_to_wrap' => 6 ],
			'nueve4_blog_post_meta_heading'        => [ 'controls_to_wrap' => 3 ],
			'nueve4_read_more_options'             => [
				'accordion' => true,
				'expanded'  => false,
				'class'     => 'read-more-options-accordion',
			],
		];

		foreach ( $changes as $control_slug => $props ) {
			foreach ( $props as $prop => $new_value ) {
				$this->change_customizer_object( 'control', $control_slug, $prop, $new_value );
			}
		}

		$this->wpc->remove_control( 'nueve4_metadata_options' );
	}

	/**
	 * Add customize section
	 */
	private function section_blog() {
		$this->add_section(
			new Section(
				$this->section,
				array(
					'priority' => 35,
					'title'    => esc_html__( 'Blog / Archive', 'nueve4' ),
					'panel'    => 'nueve4_layout',
				)
			)
		);
	}

	/**
	 * Add blog layout controls.
	 */
	private function add_layout_controls() {
		$this->add_control(
			new Control(
				'nueve4_blog_layout_heading',
				array(
					'sanitize_callback' => 'sanitize_text_field',
				),
				array(
					'label'            => esc_html__( 'Blog Layout', 'nueve4' ),
					'section'          => $this->section,
					'priority'         => 10,
					'class'            => 'blog-layout-accordion',
					'accordion'        => true,
					'controls_to_wrap' => 6,
				),
				'Nueve4\Customizer\Controls\Heading'
			)
		);

		$this->add_control(
			new Control(
				$this->section,
				[
					'default'           => 'grid',
					'sanitize_callback' => [ $this, 'sanitize_blog_layout' ],
				],
				[
					'section'  => $this->section,
					'priority' => 11,
					'choices'  => [
						'default' => [
							'name'  => __( 'List', 'nueve4' ),
							'image' => 'data:image/png;base64,',
						],
						'covers'  => [
							'name'  => __( 'Covers', 'nueve4' ),
							'image' => 'data:image/png;base64,',
						],
						'grid'    => [
							'name'  => __( 'Grid', 'nueve4' ),
							'image' => 'data:image/jpeg;base64,',
						],
					],
				],
				'\Nueve4\Customizer\Controls\React\Radio_Image'
			)
		);

		$grid_layout_default = '{"desktop":3,"tablet":2,"mobile":1}';

		$this->add_control(
			new Control(
				'nueve4_grid_layout',
				array(
					'sanitize_callback' => 'nueve4_sanitize_range_value',
					'default'           => $grid_layout_default,
				),
				array(
					'label'           => esc_html__( 'Columns', 'nueve4' ),
					'section'         => $this->section,
					'units'           => array(
						'items',
					),
					'input_attrs'     => [
						'min'        => 1,
						'max'        => 4,
						'defaultVal' => json_decode( $grid_layout_default, true ),
					],
					'priority'        => 11,
					'active_callback' => array( $this, 'is_column_layout' ),
				),
				'Nueve4\Customizer\Controls\React\Responsive_Range'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_blog_covers_text_color',
				array(
					'sanitize_callback' => 'nueve4_sanitize_colors',
					'default'           => '#ffffff',
					'transport'         => 'postMessage',
				),
				array(
					'label'                 => esc_html__( 'Text Color', 'nueve4' ),
					'section'               => $this->section,
					'priority'              => 15,
					'default'               => '#ffffff',
					'active_callback'       => function () {
						return get_theme_mod( $this->section ) === 'covers';
					},
					'live_refresh_selector' => true,
					'live_refresh_css_prop' => [
						'cssVar' => [
							'vars'     => '--color',
							'selector' => '.nueve4-main',
						],
					],
				),
				'Nueve4\Customizer\Controls\React\Color'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_archive_hide_title',
				[
					'sanitize_callback' => 'nueve4_sanitize_checkbox',
					'default'           => false,
				],
				[
					'label'           => esc_html__( 'Disable Title', 'nueve4' ),
					'section'         => $this->section,
					'type'            => 'nueve4_toggle_control',
					'priority'        => 16,
					'active_callback' => function() {
						return get_option( 'show_on_front' ) !== 'posts';
					},
				],
				'Nueve4\Customizer\Controls\Checkbox'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_blog_list_alternative_layout',
				array(
					'sanitize_callback' => 'nueve4_sanitize_checkbox',
					'default'           => false,
				),
				array(
					'type'            => 'nueve4_toggle_control',
					'priority'        => 17,
					'section'         => $this->section,
					'label'           => esc_html__( 'Alternating layout', 'nueve4' ),
					'active_callback' => function () {
						$is_list_layout = get_theme_mod( $this->section ) === 'default';
						$has_image      = true;
						if ( $is_list_layout && defined( 'NEVE_PRO_VERSION' ) ) {
							$has_image = get_theme_mod( 'nueve4_blog_list_image_position', 'left' ) !== 'no';
						}
						return $is_list_layout && $has_image;
					},
				)
			)
		);

		$this->add_control(
			new Control(
				'nueve4_enable_masonry',
				array(
					'sanitize_callback' => 'nueve4_sanitize_checkbox',
					'default'           => false,
				),
				array(
					'type'            => 'nueve4_toggle_control',
					'priority'        => 35,
					'section'         => $this->section,
					'label'           => esc_html__( 'Enable Masonry', 'nueve4' ),
					'active_callback' => array( $this, 'should_show_masonry' ),
				)
			)
		);
	}

	/**
	 * Add featured post controls.
	 */
	private function add_featured_post() {
		$this->add_control(
			new Control(
				'nueve4_featured_post_heading',
				[
					'sanitize_callback' => 'sanitize_text_field',
				],
				[
					'label'            => esc_html__( 'Featured Post', 'nueve4' ),
					'section'          => $this->section,
					'priority'         => 40,
					'class'            => 'featured-post-accordion',
					'accordion'        => true,
					'expanded'         => false,
					'controls_to_wrap' => 2,
				],
				'Nueve4\Customizer\Controls\Heading'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_enable_featured_post',
				[
					'sanitize_callback' => 'nueve4_sanitize_checkbox',
					'default'           => false,
				],
				[
					'label'    => esc_html__( 'Enable featured post section', 'nueve4' ),
					'section'  => $this->section,
					'type'     => 'nueve4_toggle_control',
					'priority' => 41,
				],
				'Nueve4\Customizer\Controls\Checkbox'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_featured_post_target',
				array(
					'default'           => 'latest',
					'sanitize_callback' => array( $this, 'sanitize_featured_post_target' ),
				),
				array(
					'label'           => esc_html__( 'Featured Post', 'nueve4' ),
					'section'         => $this->section,
					'priority'        => 42,
					'type'            => 'select',
					'choices'         => [
						'latest' => esc_html__( 'Latest Post', 'nueve4' ),
						'sticky' => esc_html__( 'Sticky Post', 'nueve4' ),
					],
					'active_callback' => function() {
						return get_theme_mod( 'nueve4_enable_featured_post', false );
					},
				)
			)
		);
	}

	/**
	 * Add content ordering and controls.
	 */
	private function add_content_ordering_controls() {
		$this->add_control(
			new Control(
				'nueve4_blog_ordering_content_heading',
				array(
					'sanitize_callback' => 'sanitize_text_field',
				),
				array(
					'label'            => esc_html__( 'Ordering and Content', 'nueve4' ),
					'section'          => $this->section,
					'priority'         => 50,
					'class'            => 'blog-layout-ordering-content-accordion',
					'accordion'        => true,
					'expanded'         => false,
					'controls_to_wrap' => 4,
				),
				'Nueve4\Customizer\Controls\Heading'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_pagination_type',
				array(
					'default'           => 'number',
					'sanitize_callback' => array( $this, 'sanitize_pagination_type' ),
				),
				array(
					'label'    => esc_html__( 'Post Pagination', 'nueve4' ),
					'section'  => $this->section,
					'priority' => 53,
					'type'     => 'select',
					'choices'  => array(
						'number'   => esc_html__( 'Number', 'nueve4' ),
						'infinite' => esc_html__( 'Infinite Scroll', 'nueve4' ),
						'jump-to'  => esc_html__( 'Number', 'nueve4' ) . ' & ' . esc_html__( 'Search Field', 'nueve4' ),
					),
				)
			)
		);

		$order_default_components = array(
			'thumbnail',
			'title-meta',
			'excerpt',
		);

		$components = array(
			'thumbnail'  => __( 'Thumbnail', 'nueve4' ),
			'title-meta' => __( 'Title & Meta', 'nueve4' ),
			'excerpt'    => __( 'Excerpt', 'nueve4' ),
		);

		$this->add_control(
			new Control(
				'nueve4_post_content_ordering',
				array(
					'sanitize_callback' => array( $this, 'sanitize_post_content_ordering' ),
					'default'           => wp_json_encode( $order_default_components ),
				),
				array(
					'label'      => esc_html__( 'Post Content Order', 'nueve4' ),
					'section'    => $this->section,
					'components' => $components,
					'priority'   => 55,
				),
				'Nueve4\Customizer\Controls\React\Ordering'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_post_excerpt_length',
				array(
					'sanitize_callback' => 'nueve4_sanitize_range_value',
					'default'           => 25,
				),
				array(
					'label'       => esc_html__( 'Excerpt Length', 'nueve4' ),
					'section'     => $this->section,
					'type'        => 'nueve4_range_control',
					'input_attrs' => [
						'min'        => 5,
						'max'        => 300,
						'defaultVal' => 25,
						'step'       => 5,
					],
					'priority'    => 58,
				),
				'Nueve4\Customizer\Controls\React\Range'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_post_thumbnail_box_shadow',
				array(
					'sanitize_callback' => 'absint',
					'default'           => 0,
				),
				array(
					'label'       => esc_html__( 'Thumbnail Shadow', 'nueve4' ),
					'section'     => $this->section,
					'type'        => 'nueve4_range_control',
					'step'        => 1,
					'input_attrs' => [
						'min'        => 0,
						'max'        => 5,
						'defaultVal' => 0,
					],
					'priority'    => 59,
				),
				'Nueve4\Customizer\Controls\React\Range'
			)
		);
	}

	/**
	 * Add controls for post meta.
	 */
	private function add_post_meta_controls() {
		$this->add_control(
			new Control(
				'nueve4_blog_post_meta_heading',
				array(
					'sanitize_callback' => 'sanitize_text_field',
				),
				array(
					'label'            => esc_html__( 'Post Meta', 'nueve4' ),
					'section'          => $this->section,
					'priority'         => 70,
					'class'            => 'blog-layout-post-meta-accordion',
					'accordion'        => true,
					'controls_to_wrap' => 5,
					'expanded'         => false,
				),
				'Nueve4\Customizer\Controls\Heading'
			)
		);


		$default       = wp_json_encode( [ 'author', 'date', 'comments' ] );
		$default_value = nueve4_get_default_meta_value( 'nueve4_post_meta_ordering', $default );
		$this->add_control(
			new Control(
				'nueve4_blog_post_meta_fields',
				[
					'sanitize_callback' => 'nueve4_sanitize_meta_repeater',
					'default'           => wp_json_encode( $default_value ),
				],
				[
					'label'            => esc_html__( 'Meta Order', 'nueve4' ),
					'section'          => $this->section,
					'fields'           => [
						'hide_on_mobile' => [
							'type'  => 'checkbox',
							'label' => __( 'Hide on mobile', 'nueve4' ),
						],
					],
					'components'       => apply_filters(
						'nueve4_meta_filter',
						array(
							'author'   => __( 'Author', 'nueve4' ),
							'category' => __( 'Category', 'nueve4' ),
							'date'     => __( 'Date', 'nueve4' ),
							'comments' => __( 'Comments', 'nueve4' ),
						)
					),
					'allow_new_fields' => 'no',
					'priority'         => 71,
					'active_callback'  => [ $this, 'should_show_meta_order' ],
				],
				'\Nueve4\Customizer\Controls\React\Repeater'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_metadata_separator',
				array(
					'sanitize_callback' => 'sanitize_text_field',
					'default'           => esc_html( '/' ),
				),
				array(
					'priority'    => 72,
					'section'     => $this->section,
					'label'       => esc_html__( 'Separator', 'nueve4' ),
					'description' => esc_html__( 'For special characters make sure to use Unicode. For example > can be displayed using \003E.', 'nueve4' ),
					'type'        => 'text',
				)
			)
		);

		$this->add_control(
			new Control(
				'nueve4_author_avatar',
				array(
					'sanitize_callback' => 'nueve4_sanitize_checkbox',
					'default'           => false,
				),
				array(
					'label'    => esc_html__( 'Show Author Avatar', 'nueve4' ),
					'section'  => $this->section,
					'type'     => 'nueve4_toggle_control',
					'priority' => 73,
				)
			)
		);

		$this->add_control(
			new Control(
				'nueve4_author_avatar_size',
				array(
					'sanitize_callback' => 'nueve4_sanitize_range_value',
					'default'           => '{ "mobile": 20, "tablet": 20, "desktop": 20 }',
				),
				array(
					'label'           => esc_html__( 'Avatar Size', 'nueve4' ),
					'section'         => $this->section,
					'units'           => array(
						'px',
					),
					'input_attr'      => array(
						'mobile'  => array(
							'min'          => 20,
							'max'          => 50,
							'default'      => 20,
							'default_unit' => 'px',
						),
						'tablet'  => array(
							'min'          => 20,
							'max'          => 50,
							'default'      => 20,
							'default_unit' => 'px',
						),
						'desktop' => array(
							'min'          => 20,
							'max'          => 50,
							'default'      => 20,
							'default_unit' => 'px',
						),
					),
					'input_attrs'     => [
						'min'        => self::RELATIVE_CSS_UNIT_SUPPORTED_MIN_VALUE,
						'max'        => 50,
						'defaultVal' => [
							'mobile'  => 20,
							'tablet'  => 20,
							'desktop' => 20,
							'suffix'  => [
								'mobile'  => 'px',
								'tablet'  => 'px',
								'desktop' => 'px',
							],
						],
						'units'      => [ 'px', 'em', 'rem' ],
					],
					'priority'        => 74,
					'active_callback' => function () {
						return get_theme_mod( 'nueve4_author_avatar', false );
					},
					'responsive'      => true,
				),
				'Nueve4\Customizer\Controls\React\Responsive_Range'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_show_last_updated_date',
				array(
					'sanitize_callback' => 'nueve4_sanitize_checkbox',
					'default'           => false,
				),
				array(
					'label'    => esc_html__( 'Use last updated date instead of the published one', 'nueve4' ),
					'section'  => $this->section,
					'type'     => 'nueve4_toggle_control',
					'priority' => 85,
				)
			)
		);
	}

	/**
	 * Sanitize the container layout value
	 *
	 * @param string $value value from the control.
	 *
	 * @return string
	 */
	public function sanitize_blog_layout( $value ) {
		$allowed_values = array( 'default', 'covers', 'grid' );
		if ( ! in_array( $value, $allowed_values, true ) ) {
			return 'grid';
		}

		return sanitize_text_field( $value );
	}

	/**
	 * Sanitize the pagination type
	 *
	 * @param string $value value from the control.
	 *
	 * @return string
	 */
	public function sanitize_pagination_type( $value ) {
		$allowed_values = array( 'number', 'infinite', 'jump-to' );
		if ( ! in_array( $value, $allowed_values, true ) ) {
			return 'number';
		}

		return esc_html( $value );
	}

	/**
	 * Sanitize featured post target
	 *
	 * @param string $value value from the control.
	 *
	 * @return string
	 */
	public function sanitize_featured_post_target( $value ) {
		$allowed_values = [ 'sticky', 'latest' ];
		if ( ! in_array( $value, $allowed_values, true ) ) {
			return 'latest';
		}

		return esc_html( $value );
	}

	/**
	 * Sanitize content order control.
	 */
	public function sanitize_post_content_ordering( $value ) {
		$allowed = array(
			'thumbnail',
			'title-meta',
			'excerpt',
		);

		if ( empty( $value ) ) {
			return wp_json_encode( $allowed );
		}

		$decoded = json_decode( $value, true );

		foreach ( $decoded as $val ) {
			if ( ! in_array( $val, $allowed, true ) ) {
				return wp_json_encode( $allowed );
			}
		}

		return $value;
	}

	/**
	 * Callback to show the meta order control.
	 *
	 * @return bool
	 */
	public function should_show_meta_order() {
		$default       = array(
			'thumbnail',
			'title-meta',
			'excerpt',
		);
		$content_order = get_theme_mod( 'nueve4_post_content_ordering', wp_json_encode( $default ) );
		$content_order = json_decode( $content_order, true );
		if ( ! in_array( 'title-meta', $content_order, true ) ) {
			return false;
		}

		return true;
	}

	/**
	 * Callback to show grid columns control.
	 *
	 * @return bool
	 */
	public function is_column_layout() {
		$blog_layout = get_theme_mod( $this->section, 'grid' );

		return in_array( $blog_layout, [ 'grid', 'covers' ], true );
	}

	/**
	 * Callback to show masonry control.
	 *
	 * @return bool
	 */
	public function should_show_masonry() {
		if ( ! $this->is_column_layout() ) {
			return false;
		}

		$columns = json_decode( get_theme_mod( 'nueve4_grid_layout', $this->grid_columns_default() ), true );
		$columns = array_filter(
			array_values( $columns ),
			function ( $value ) {
				return $value > 1;
			}
		);

		if ( empty( $columns ) ) {
			return false;
		}

		return true;
	}

	/**
	 * Add typography shortcut.
	 */
	private function add_typography_shortcut() {
		$this->add_control(
			new Control(
				'nueve4_blog_typography_shortcut',
				array(
					'sanitize_callback' => 'nueve4_sanitize_text_field',
				),
				array(
					'button_class'     => 'nv-top-bar-menu-shortcut',
					'text_before'      => __( 'Customize Typography for the Archive page', 'nueve4' ),
					'text_after'       => '.',
					'button_text'      => __( 'here', 'nueve4' ),
					'is_button'        => false,
					'control_to_focus' => 'nueve4_archive_typography_post_title_accordion_wrap',
					'shortcut'         => true,
					'section'          => $this->section,
					'priority'         => 1000,
				),
				'\Nueve4\Customizer\Controls\Button'
			)
		);
	}
}
