<?php
/**
 * Customizer upsells controls.
 *
 * @package Nueve4\Customizer\Options
 */

namespace Nueve4\Customizer\Options;

use Nueve4\Core\Theme_Info;
use Nueve4\Customizer\Base_Customizer;
use Nueve4\Customizer\Types\Section;
use Nueve4\Customizer\Types\Control;

/**
 * Class Upsells
 *
 * @package Nueve4\Customizer\Options
 */
class Upsells extends Base_Customizer {

	use Theme_Info;

	/**
	 * Holds the main upsell url
	 *
	 * @var string
	 */
	private $upsell_url = '';

	/**
	 * Init function
	 *
	 * @return bool|void
	 */
	public function init() {
		if ( $this->has_valid_addons() ) {
			return;
		}

		$this->upsell_url = esc_url_raw( apply_filters( 'nueve4_upgrade_link_from_child_theme_filter', tsdk_utmify( 'https://themeisle.com/themes/nueve4/upgrade/', 'learnmorebtn' ) ) );

		parent::init();
		add_action( 'customize_controls_enqueue_scripts', array( $this, 'localize_upsell' ) );

		add_filter( 'theme_mod_nueve4_checkout_page_layout', array( $this, 'override_nueve4_checkout_page_layout_theme_mod' ), 10, 1 );
	}


	/**
	 * Dismiss the upsell banner for 7 days.
	 *
	 * @return void
	 */
	public static function remove_customizer_upsell_notice() {
		check_ajax_referer( 'nueve4-upsell-banner-nonce', 'nonce' );

		set_transient( 'upsell_dismiss_banner_customizer', true, 7 * DAY_IN_SECONDS );
		wp_send_json( [] );
	}

	/**
	 * Overrides nueve4_checkout_page_layout theme mod as 'standard' if Nueve4 Pro addon is disabled or not activated.
	 *
	 * @param  string $value current value.
	 * @return string
	 */
	public function override_nueve4_checkout_page_layout_theme_mod( $value ) {
		return 'standard';
	}

	/**
	 * Localize upsell script and send strings.
	 */
	public function localize_upsell() {
		wp_localize_script(
			'nueve4-customizer-controls',
			'upsellConfig',
			array(
				'button_url'    => esc_url_raw( apply_filters( 'nueve4_upgrade_link_from_child_theme_filter', tsdk_utmify( 'https://themeisle.com/themes/nueve4/upgrade/', 'proheader' ) ) ),
				'button_text'   => esc_html__( 'Get the PRO version!', 'nueve4' ),
				'text'          => esc_html__( 'Extend your header with more components and settings, build sticky/transparent headers or display them conditionally.', 'nueve4' ),
				'screen_reader' => esc_html__( '(opens in a new tab)', 'nueve4' ),
			)
		);
	}

	/**
	 * Function that should be extended to add customizer controls.
	 *
	 * @return void
	 */
	public function add_controls() {
		$this->wpc->register_section_type( '\Nueve4\Customizer\Controls\Simple_Upsell_Section' );
		$this->wpc->register_section_type( '\Nueve4\Customizer\Controls\React\Upsell_Banner_Section' );
		$this->wpc->register_control_type( '\Nueve4\Customizer\Controls\React\Upsell_Banner' );
		$this->section_upsells();
		$this->control_upsells();
		$this->checkout_locked_layout();
		$this->product_catalog_upsells();
	}

	/**
	 * Locked Checkout Layouts
	 *
	 * @return void
	 */
	private function checkout_locked_layout() {
		$this->add_control(
			new Control(
				'nueve4_checkout_page_layout',
				[
					'default'           => 'standard',
					'sanitize_callback' => [ $this, 'sanitize_checkout_layout' ],
				],
				[
					'section'  => 'woocommerce_checkout',
					'priority' => 5,
					'choices'  => [
						'standard' => [
							'name'  => esc_html__( 'Standard', 'nueve4' ),
							'image' => 'data:image/png;base64,',
						],
						'vertical' => [
							'name'      => esc_html__( 'Vertical', 'nueve4' ),
							'image'     => 'data:image/png;base64,',
							'upsellUrl' => tsdk_utmify( 'https://themeisle.com/themes/nueve4/upgrade/', 'woocheckoutvertical' ),
						],
						'stepped'  => [
							'name'      => esc_html__( 'Stepped', 'nueve4' ),
							'image'     => 'data:image/png;base64,',
							'upsellUrl' => tsdk_utmify( 'https://themeisle.com/themes/nueve4/upgrade/', 'woocheckoutstepped' ),
						],
					],
				],
				'\Nueve4\Customizer\Controls\React\Radio_Image'
			)
		);
	}

	/**
	 * Locked options on Product Catalog
	 *
	 * @return void
	 */
	private function product_catalog_upsells() {
		$this->add_control(
			new Control(
				'nueve4_product_card_layout_ui_heading',
				[
					'sanitize_callback' => 'sanitize_text_field',
					'transport'         => $this->selective_refresh,
				],
				[
					'label'            => esc_html__( 'Layout', 'nueve4' ),
					'section'          => 'woocommerce_product_catalog',
					'priority'         => 200,
					'class'            => 'card-layout-accordion',
					'accordion'        => true,
					'controls_to_wrap' => 2,
					'expanded'         => false,
				],
				'Nueve4\Customizer\Controls\Heading'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_product_card_layout',
				[
					'transport'         => 'postMessage',
					'default'           => 'grid',
					'sanitize_callback' => function() {
						return 'grid';
					},
				],
				[
					'label'    => esc_html__( 'Layout', 'nueve4' ),
					'section'  => 'woocommerce_product_catalog',
					'priority' => 210,
					'choices'  => [
						'grid' => [
							'name' => __( 'Grid', 'nueve4' ),
							'url'  => 'data:image/png;base64,',
						],
						'list' => [
							'name'      => __( 'List', 'nueve4' ),
							'url'       => 'data:image/png;base64,',
							'upsellUrl' => tsdk_utmify( 'https://themeisle.com/themes/nueve4/upgrade/', 'woocardlistlayout' ),
						],
					],
				],
				'\Nueve4\Customizer\Controls\React\Radio_Image'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_add_to_cart_display',
				[
					'default'           => 'none',
					'sanitize_callback' => function() {
						return 'none';
					},
				],
				[
					'label'    => esc_html__( 'Button Style', 'nueve4' ),
					'section'  => 'woocommerce_product_catalog',
					'priority' => 230,
					'choices'  => [
						'none'     => [
							'name' => __( 'None', 'nueve4' ),
							'url'  => 'data:image/png;base64,',
						],
						'after'    => [
							'name'      => __( 'Bottom', 'nueve4' ),
							'url'       => 'data:image/png;base64,',
							'upsellUrl' => tsdk_utmify( 'https://themeisle.com/themes/nueve4/upgrade/', 'wooaddtocartafter' ),
						],
						'on_image' => [
							'name'      => __( 'Overlay', 'nueve4' ),
							'url'       => 'data:image/png;base64,',
							'upsellUrl' => tsdk_utmify( 'https://themeisle.com/themes/nueve4/upgrade/', 'wooaddtocartonimage' ),
						],
					],
				],
				'\Nueve4\Customizer\Controls\React\Radio_Image'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_category_card_layout_ui_heading',
				[
					'sanitize_callback' => 'sanitize_text_field',
					'transport'         => $this->selective_refresh,
				],
				[
					'label'            => esc_html__( 'Category', 'nueve4' ),
					'section'          => 'woocommerce_product_catalog',
					'priority'         => 300,
					'class'            => 'category-layout-accordion',
					'accordion'        => true,
					'controls_to_wrap' => 1,
					'expanded'         => false,
				],
				'Nueve4\Customizer\Controls\Heading'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_category_card_layout',
				[
					'default'           => 'default',
					'sanitize_callback' => function() {
						return 'default';
					},
				],
				[
					'label'    => esc_html__( 'Layout', 'nueve4' ),
					'section'  => 'woocommerce_product_catalog',
					'priority' => 310,
					'choices'  => [
						'default' => [
							'name' => 'Simple',
							'url'  => 'data:image/svg+xml;base64,',
						],
						'style-3' => [
							'name'      => 'Boxed',
							'url'       => 'data:image/svg+xml;base64,',
							'upsellUrl' => tsdk_utmify( 'https://themeisle.com/themes/nueve4/upgrade/', 'woocategorycardboxed' ),
						],
						'style-2' => [
							'name'      => 'Hover',
							'url'       => 'data:image/svg+xml;base64,',
							'upsellUrl' => tsdk_utmify( 'https://themeisle.com/themes/nueve4/upgrade/', 'woocategorycardhover' ),
						],
					],
				],
				'\Nueve4\Customizer\Controls\React\Radio_Image'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_sale_tag_ui_heading',
				[
					'sanitize_callback' => 'sanitize_text_field',
					'transport'         => $this->selective_refresh,
				],
				[
					'label'            => esc_html__( 'Tags', 'nueve4' ),
					'section'          => 'woocommerce_product_catalog',
					'priority'         => 600,
					'class'            => 'sale-tag-accordion',
					'accordion'        => true,
					'controls_to_wrap' => 1,
					'expanded'         => false,
				],
				'Nueve4\Customizer\Controls\Heading'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_sale_tag_position',
				[
					'default'           => 'inside',
					'sanitize_callback' => function() {
						return 'inside';
					},
				],
				[
					'label'         => esc_html__( 'Style', 'nueve4' ),
					'section'       => 'woocommerce_product_catalog',
					'priority'      => 610,
					'documentation' => [
						'link' => 'https://docs.themeisle.com/article/1058-woocommerce-booster-documentation#sale-tag',
					],
					'choices'       => [
						'inside'  => [
							'url' => 'data:image/png;base64,',
						],
						'outside' => [
							'url'       => 'data:image/png;base64,',
							'upsellUrl' => tsdk_utmify( 'https://themeisle.com/themes/nueve4/upgrade/', 'woosaletagoutside' ),
						],
					],
				],
				'\Nueve4\Customizer\Controls\React\Radio_Image'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_card_content_ui_heading',
				[
					'sanitize_callback' => 'sanitize_text_field',
					'transport'         => $this->selective_refresh,
				],
				[
					'label'            => esc_html__( 'Content', 'nueve4' ),
					'section'          => 'woocommerce_product_catalog',
					'priority'         => 500,
					'class'            => 'card-content-accordion',
					'accordion'        => true,
					'controls_to_wrap' => 1,
					'expanded'         => false,
				],
				'Nueve4\Customizer\Controls\Heading'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_product_content_alignment',
				[
					'default'           => 'left',
					'sanitize_callback' => function() {
						return 'left';
					},
				],
				[
					'label'    => esc_html__( 'Alignment', 'nueve4' ),
					'section'  => 'woocommerce_product_catalog',
					'priority' => 520,
					'choices'  => [
						'left'   => [
							'name' => __( 'Left', 'nueve4' ),
							'url'  => 'data:image/png;base64,',
						],
						'center' => [
							'name'      => __( 'Center', 'nueve4' ),
							'url'       => 'data:image/png;base64,',
							'upsellUrl' => tsdk_utmify( 'https://themeisle.com/themes/nueve4/upgrade/', 'wooscardcontentalingcenter' ),
						],
						'right'  => [
							'name'      => __( 'Right', 'nueve4' ),
							'url'       => 'data:image/png;base64,',
							'upsellUrl' => tsdk_utmify( 'https://themeisle.com/themes/nueve4/upgrade/', 'wooscardcontentalingright' ),
						],
						'inline' => [
							'name'      => __( 'Left', 'nueve4' ) . ' / ' . __( 'Right', 'nueve4' ),
							'url'       => 'data:image/png;base64,',
							'upsellUrl' => tsdk_utmify( 'https://themeisle.com/themes/nueve4/upgrade/', 'wooscardcontentalinginline' ),
						],
					],
				],
				'\Nueve4\Customizer\Controls\React\Radio_Image'
			)
		);
	}

	/**
	 * Sanitize checkout layout that forcefully updates the selection as standard.
	 *
	 * @return string
	 */
	public function sanitize_checkout_layout() {
		return 'standard';
	}

	/**
	 * Add upsells section
	 */
	private function section_upsells() {

		$this->add_section(
			new Section(
				'nueve4_free_pro_upsell',
				array(
					'priority' => -100,
					'title'    => esc_html__( 'Nueve4 PRO Features', 'nueve4' ),
					'url'      => $this->upsell_url,
				),
				'Nueve4\Customizer\Controls\React\Upsell_Section'
			)
		);

		$this->add_section(
			new Section(
				'nueve4_scroll_to_top_upsell',
				array(
					'priority' => 80,
					'title'    => esc_html__( 'Scroll To Top', 'nueve4' ),
					'panel'    => 'nueve4_layout',
				)
			)
		);
	}

	/**
	 * Add upsells controls
	 */
	private function control_upsells() {
		$this->add_control(
			new Control(
				'nueve4_upsell_main_control',
				[ 'sanitize_callback' => 'sanitize_text_field' ],
				[
					'text'        => esc_html__( 'Nueve4 PRO Features', 'nueve4' ),
					'button_text' => esc_html__( 'Learn More', 'nueve4' ),
					'section'     => 'nueve4_free_pro_upsell',
					'priority'    => PHP_INT_MIN,
					'link'        => $this->upsell_url,
				],
				'Nueve4\Customizer\Controls\Simple_Upsell'
			)
		);


		$this->add_control(
			new Control(
				'nueve4_scroll_to_top_cta_control',
				[ 'sanitize_callback' => 'sanitize_text_field' ],
				[
					/* translators: Module name for the upsell. */
					'text'        => sprintf( __( 'Unlock %s with the Pro version.', 'nueve4' ), __( 'Scroll To Top', 'nueve4' ) ),
					'button_text' => esc_html__( 'Get the PRO version!', 'nueve4' ),
					'section'     => 'nueve4_scroll_to_top_upsell',
					'priority'    => PHP_INT_MIN,
					'link'        => $this->upsell_url,
					'class'       => 'column-layout',
					'use_primary' => 'true',
				],
				'Nueve4\Customizer\Controls\Simple_Upsell'
			)
		);

		$this->add_control(
			new Control(
				'nueve4_scroll_to_top_upsell_control',
				[ 'sanitize_callback' => 'sanitize_text_field' ],
				[
					'section'  => 'nueve4_scroll_to_top_upsell',
					'priority' => 2,
				],
				'Nueve4\Customizer\Controls\Upsells\Scroll_To_Top_Control'
			)
		);

		$upsells         = [];
		$upsells_banners = [];

		$upsells_banners['blog_archive'] = [
			'text'        => __( 'More blog layout customization options available in PRO', 'nueve4' ),
			'button_text' => __( 'Learn More', 'nueve4' ),
			'use_logo'    => true,
			'section'     => 'nueve4_blog_archive_layout',
		];
		$upsells_banners['single_post']  = [
			'text'        => __( 'More single post components available in PRO', 'nueve4' ),
			'button_text' => __( 'Learn More', 'nueve4' ),
			'use_logo'    => true,
			'section'     => 'nueve4_single_post_layout',
		];


		$hfg_header                     = 'hfg_header';
		$hfg_header_text                = __( 'Extend your header with more components and settings, build sticky/transparent headers or display them conditionally.', 'nueve4' );
		$hfg_header_button              = __( 'Get the PRO version!', 'nueve4' );
		$upsells_banners[ $hfg_header ] = [
			'text'        => $hfg_header_text,
			'button_text' => $hfg_header_button,
			'use_logo'    => true,
			'panel'       => $hfg_header,
			'type'        => 'section',
		];

		foreach ( [ 'top', 'main', 'bottom', 'sidebar' ] as $section ) {
			$section_id                     = $hfg_header . '_layout_' . $section;
			$upsells_banners[ $section_id ] = [
				'text'        => $hfg_header_text,
				'button_text' => $hfg_header_button,
				'use_logo'    => true,
				'section'     => $section_id,
			];
		}

		$hfg_footer                     = 'hfg_footer';
		$hfg_footer_text                = __( 'Nueve4 PRO Features', 'nueve4' );
		$hfg_footer_button              = __( 'Get the PRO version!', 'nueve4' );
		$upsells_banners[ $hfg_footer ] = [
			'text'        => $hfg_footer_text,
			'button_text' => $hfg_footer_button,
			'use_logo'    => true,
			'panel'       => $hfg_footer,
			'type'        => 'section',
		];
		foreach ( [ 'top', 'main', 'bottom' ] as $section ) {
			$section_id                     = $hfg_footer . '_layout_' . $section;
			$upsells_banners[ $section_id ] = [
				'text'        => $hfg_footer_text,
				'button_text' => $hfg_footer_button,
				'use_logo'    => true,
				'section'     => $section_id,
			];
		}


		if ( class_exists( 'WooCommerce', false ) ) {
			$upsells['product_catalog']       = [
				'text'        => __( 'More product catalog options available in PRO', 'nueve4' ) . ' (Business & Agency)',
				'button_text' => __( 'Learn More', 'nueve4' ),
				'section'     => 'woocommerce_product_catalog',
			];
			$upsells['woocommerce_checkout']  = [
				'text'        => __( 'More checkout options available in PRO', 'nueve4' ) . ' (Business & Agency)',
				'button_text' => __( 'Learn More', 'nueve4' ),
				'section'     => 'woocommerce_checkout',
			];
			$upsells['single_product_layout'] = [
				'text'        => __( 'More single product options available in PRO', 'nueve4' ) . ' (Business & Agency)',
				'button_text' => __( 'Learn More', 'nueve4' ),
				'section'     => 'nueve4_single_product_layout',
			];
			$upsells['typography']            = [
				'text'        => __( 'WooCommerce typography options available in PRO', 'nueve4' ) . ' (Business & Agency)',
				'button_text' => __( 'Learn More', 'nueve4' ),
				'panel'       => 'nueve4_typography',
				'type'        => 'section',
			];
		}

		$is_dismissed = get_transient( 'upsell_dismiss_banner_customizer' );
		if ( $is_dismissed === false ) {
			foreach ( $upsells_banners as $id => $args ) {
				if ( isset( $args['type'] ) && $args['type'] === 'section' ) {
					$section_id = 'nueve4_' . $id . '_upsell_section';
					$this->add_section(
						new Section(
							$section_id,
							array_merge(
								$args,
								[
									'type'     => 'nueve4_upsell_banner_section',
									'priority' => 10000,
									'nonce'    => wp_create_nonce( 'nueve4-upsell-banner-nonce' ),
									'url'      => tsdk_utmify( 'https://themeisle.com/themes/nueve4/upgrade/', 'panel-' . $args['panel'] ),
								]
							),
							'\Nueve4\Customizer\Controls\React\Upsell_Banner_Section'
						)
					);

					continue;
				}
				$control_id = 'nueve4_' . $id . '_upsell_banner_control';
				$this->add_control(
					new Control(
						$control_id,
						[ 'sanitize_callback' => 'sanitize_text_field' ],
						array_merge(
							$args,
							[
								'type'     => 'nueve4_upsell_banner',
								'priority' => 10000,
								'nonce'    => wp_create_nonce( 'nueve4-upsell-banner-nonce' ),
								'url'      => tsdk_utmify( 'https://themeisle.com/themes/nueve4/upgrade/', 'upsell-banner-customizer-section-' . $args['section'] ),
							]
						),
						'\Nueve4\Customizer\Controls\React\Upsell_Banner'
					)
				);
			}
		}

		foreach ( $upsells as $id => $args ) {
			if ( isset( $args['type'] ) && $args['type'] === 'section' ) {
				$this->add_section(
					new Section(
						'nueve4_' . $id . '_upsell_section',
						array_merge(
							$args,
							[
								'type'     => 'nv_simple_upsell_section',
								'priority' => 10000,
								'link'     => tsdk_utmify( 'https://themeisle.com/themes/nueve4/upgrade/', 'panel-' . $args['panel'] ),
							]
						),
						'\Nueve4\Customizer\Controls\Simple_Upsell_Section'
					)
				);

				continue;
			}
			$this->add_control(
				new Control(
					'nueve4_' . $id . '_upsell',
					[ 'sanitize_callback' => 'sanitize_text_field' ],
					array_merge(
						$args,
						[
							'priority' => 10000,
							'link'     => tsdk_utmify( 'https://themeisle.com/themes/nueve4/upgrade/', 'section-' . $args['section'] ),
						]
					),
					'Nueve4\Customizer\Controls\Simple_Upsell'
				)
			);
		}
	}
}
