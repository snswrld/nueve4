 ...
 * @package Nueve4
 */
$title_style = apply_filters( 'nueve4_title_alignment_style', '', 'normal' );
...
$specific_hide_title = get_post_meta( $pid, 'nueve4_meta_disable_title', true );

if ( (int) get_option( 'page_for_posts' ) === $pid || is_archive() ) {
	$hide_title = get_theme_mod( 'nueve4_archive_hide_title', false );
}
...
<div class="nv-page-title-wrap <?php echo esc_attr( $args['wrap-class'] ); ?>" <?php echo wp_kses_post( $title_style ); ?>>
	<div class="nv-page-title <?php echo esc_attr( $args['class'] ); ?>">
		<?php do_action( 'nueve4_before_page_title' ); ?>
		...
		<?php } ?>
		<?php do_action( 'nueve4_after_page_title' ); ?>
	</div><!--.nv-page-title-->
