...
use Nueve4\Core\Styles\Css_Prop;
...
abstract class Abstract_Builder implements Builder {
	...
	public function __construct() {
		...
		$this->set_property( 'layout_control_id', $this->control_id . ( nueve4_is_new_builder() ? '_v2' : '' ) );
	...
	public function define_row_settings( $row_id ) {
		...
		if ( $this->columns_layout && nueve4_is_new_builder() ) {
		...
		if ( empty( $this->active_components ) ) {
			...
			foreach ( $this->get_layout_data() as $devices ) {
				foreach ( $devices as $row_index => $row ) {
					if ( nueve4_is_new_builder() && $row_index !== 'sidebar' ) {
						foreach ( $row as $slot ) {
